...
PRIOR_FILES = [
    "outputs/natural_multihop/train.jsonl",
    "outputs/natural_multihop/eval.jsonl",
    "outputs/natural_multihop_batch4/train.jsonl",
    "outputs/natural_multihop_batch4/eval.jsonl",
    "outputs/retrieval_filtered_batch3/train.jsonl",
    "outputs/retrieval_filtered_batch3/eval.jsonl",
]
...
print(f"Loaded {len(prior_questions)} existing questions for dedup")


# --- Relaxed generation prompt ---
...
# ---------------------------------------------------------------------------
# Monkey-patch Pipeline to pre-seed dedup with prior questions
# ---------------------------------------------------------------------------
...
def _patched_run(self):
    ...
    def _seeded_init(dedup_self, *args, **kwargs):
        ...
        for q in prior_questions:
            ngrams = dedup_self._compute_ngrams(q)
            dedup_self._accepted_ngrams.append(ngrams)
        print(f"  [dedup] Pre-seeded with {len(prior_questions)} prior questions")
...
cfg = PipelineConfig(
    ...
    output=OutputConfig(dir="outputs/natural_multihop_batch5"),
    micro_batch=MicroBatchConfig(resume=False, max_parallel_batches=1),
)
...
out_dir = Path("outputs/natural_multihop_batch5")
out_dir.mkdir(parents=True, exist_ok=True)

print("=" * 60)
print("NATURAL MULTI-HOP BATCH 5")
print("=" * 60)
print(f"  Target: 20 samples")
print(f"  Prior questions loaded: {len(prior_questions)} (for dedup)")
print(f"  Filters: quality_gate → retrieval_too_easy_llm → grounding_llm → hop_count_validity")
print(f"  Post-filter: naturalness judge (Sonnet)")
print("=" * 60)
...
# ===== Post-pipeline naturalness filter =====
print(f"\n{'='*60}")
